...
use gq_core::query::{query_arguments::ValueType, query_key::RawKey};
...
impl ValueKey {
    fn new(key: RawKey, r#type: String) -> Self {
        Self { key, r#type }
    }
}
...
impl From<(&String, &Value)> for ValueKey {
    fn from((key, value): (&String, &Value)) -> Self {
        let raw_key = RawKey::from(key.as_str());
        Self::new(raw_key, value.value_type())
    }
}

impl<I> From<(RawKey, I)> for CompletionItem
where
    I: IntoIterator<Item = ValueKey>,
{
    fn from((key, value_keys): (RawKey, I)) -> Self {
        let types = value_keys
            .into_iter()
            .map(|value_key| value_key.r#type)
            .join(", ");
        let detail = format!("({types})");
        CompletionItem::new(key.to_string(), Some(detail))
    }
}
